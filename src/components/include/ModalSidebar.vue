...
<div class="pop-over is-shown" :style="{left: offset.left+'px', top: offset.top+'px'}">
    <div class="no-back">
        <div class="pop-over-header js-pop-over-header">
            <a href="#" v-if="showAddLabel || showDeleteLabel" @click="backLabel" class="pop-over-header-back-btn icon-sm icon-back is-shown">
        ...
        <div>
            <div v-if="offset.type === 'label'" class="pop-over-content js-pop-over-content u-fancy-scrollbar js-tab-parent" style="max-height: 368px;">
                <div>
                    <div>
                        <div v-if="showDeleteLabel">
                            <p>Dữ liệu không thể khôi phục.</p><input class="btn-confirm full nch-button nch-button--danger" type="submit" @click="deleteLabel" value="Xoá" data-test-id="">
                        </div>
                        <div v-if="showAddLabel && !showEditLabel" class="edit-label">
                            <label for="labelName">Tên</label>
                            <input id="labelName" class="js-autofocus js-label-name" type="text" name="name" v-model="titleLabel">
                            <label>Chọn một màu</label>
                            <div class="u-clearfix">
                                <DetailLabel class="label-item" v-for="(item,index) in arrayLabels" @activeLabel="activeLabel" :key="index" :color="item" />
                            </div>
                            <div class="edit-labels-no-color-section u-clearfix">
                                ...
                                <div class="edit-labels-no-color-section-text">
                                    <p class="u-bottom">Không có màu.</p>
                                    <p class="u-bottom quiet">Điều này sẽ không hiển thị trên đầu thẻ.</p>
                                </div>
                            </div>
                            <div class="u-clearfix"><input class="nch-button nch-button--primary wide js-submit" @click="addLabelToCard" type="submit" value="Tạo mới">
                                <div class="u-float-right js-accessory-view-holder"></div>
                            </div>
                        </div>
                        <input v-if="!showAddLabel && !showDeleteLabel" class="js-autofocus label-search" v-model="nameLabel" @input="searchLabel" type="text" placeholder="Tìm nhãn..." value="" autocomplete="off">
                        <div v-if="!showAddLabel && !showDeleteLabel" class="pop-over-section">
                            <h4>Nhãn</h4>
                            <ul class="edit-labels-pop-over js-labels-list">
                                <li v-for="(label,index) in labels" :key="index"><a class="card-label-edit-button" @click="editLabel(label)"><i class="el-icon-edit"></i></a>
                                    <DetailLabel :color="label.color" @activeLabel="changeLabel" :id="label.id" :card="card" :title="label.name" />
                                </li>
                            </ul>
                            <div>
                        ...
                        <div v-if="showEditLabel && showAddLabel" class="edit-label"><label for="labelName">Tên</label><input class="js-autofocus js-label-name" type="text" name="name" v-model="titleLabel"><label>Chọn
                                một màu</label>
                            <div class="u-clearfix">
                                <DetailLabel class="label-item" v-for="(item,index) in arrayLabels" @activeLabel="activeLabel" :key="index" :color="item" :isActive="isActive" />
                            </div>
                            <div class="edit-labels-no-color-section u-clearfix">
                                <div class="edit-labels-no-color-section-color">
                                    <DetailLabel class="label-item" :color="'default'" @activeLabel="activeLabel" />
                            ...
                            <div class="u-clearfix edit-btn">
                                <input class="nch-button nch-button--primary wide js-submit" @click="updateLabel" type="submit" value="Lưu">
                                <input type="submit" value="Xoá" @click="showConfirmDelete" class="remove-label nch-button nch-button--danger">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div v-else class="pop-over-content js-pop-over-content u-fancy-scrollbar js-tab-parent" style="max-height: 451px;">
                <div>
                    <div>
                        <div class="edit-label">
                            <label for="id-checklist">Tiêu đề</label>
                            <input id="id-checklist" type="text" v-model="titleCheckList" dir="auto">
